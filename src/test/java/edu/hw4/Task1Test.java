...
class Task1Test {
    ...
    void sort() {
        ...
        // when
        var result = AnimalUtilities.sortByHeightAsc(animals);

        // then
        assertThat(result.get(FIRST).height()).isLessThanOrEqualTo(result.get(SECOND).height());
        assertThat(result.get(SECOND).height()).isLessThanOrEqualTo(result.get(THIRD).height());
        assertThat(result.get(THIRD).height()).isLessThanOrEqualTo(result.get(FOURTH).height());
    }
}
