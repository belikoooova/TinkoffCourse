...
class Task13Test {
    ...
    @Test
    @DisplayName("Find amount of animals with name that contains more than two words")
    void findAmountOfAnimalsWithNameThatContainsMoreThanTwoWords() {
        // given
        var animals = getListOfAnimals();

        // when
        var result = AnimalUtilities.getAmountOfAnimalsWithNameThatContainsMoreThanTwoWords(animals);
    ...
    private Integer naiveCount(List<Animal> animals) {
        ...
        for (var animal : animals) {
            if (animal.name().split(" ").length > WORDS_AMOUNT) {
                ++amount;
            }
        }
        return amount;
