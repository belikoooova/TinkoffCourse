...
class Task9Test {
    ...
    @Test
    @DisplayName("Find the total amount of paws")
    void findTheTotalAmountOfPaws() {
        // given
        var animals = getListOfAnimals();

        // when
        var result = AnimalUtilities.getAmountOfPaws(animals);

        // then
        assertThat(result).isEqualTo(naiveCount(animals));
    }

    private int naiveCount(List<Animal> animals) {
        int res = 0;
